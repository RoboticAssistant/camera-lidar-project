...
void train_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear)
{
    ...
    char *train_images = option_find_str(options, "train", "src/object_depth_module/data/train.list");
...
void print_imagenet_detections(FILE *fp, int id, box *boxes, float **probs, int total, int classes, int w, int h)
{
    ...
    for(i = 0; i < total; ++i){
        ...
        for(j = 0; j < classes; ++j){
            int class_t = j;
            if (probs[i][class_t]) fprintf(fp, "%d %d %f %f %f %f %f\n", id, j+1, probs[i][class_t],
                    xmin, ymin, xmax, ymax);
        }
    }
}
...
void validate_detector(char *datacfg, char *cfgfile, char *weightfile, char *outfile)
{
    int j;
    list *options = read_data_cfg(datacfg);
    char *valid_images = option_find_str(options, "valid", "src/object_depth_module/data/train.list");
    char *name_list = option_find_str(options, "names", "src/object_depth_module/data/names.list");
...
void validate_detector_recall(char *cfgfile, char *weightfile)
{
    ...
    list *plist = get_paths("src/object_depth_module/data/voc.2007.test");
    char **paths = (char **)list_to_array(plist);
