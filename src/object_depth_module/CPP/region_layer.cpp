...
void delta_region_class(float *output, float *delta, int index, int class_t, int classes, tree *hier, float scale, float *avg_cat)
{
    int i, n;
    if(hier){
        float pred = 1;
        while(class_t >= 0){
            pred *= output[index + class_t];
            int g = hier->group[class_t];
            int offset = hier->group_offset[g];
            for(i = 0; i < hier->group_size[g]; ++i){
                delta[index + offset + i] = scale * (0 - output[index + offset + i]);
            }
            delta[index + class_t] = scale * (1 - output[index + class_t]);

            class_t = hier->parent[class_t];
        }
        *avg_cat += pred;
    } else {
        for(n = 0; n < classes; ++n){
            delta[index + n] = scale * (((n == class_t)?1 : 0) - output[index + n]);
            if(n == class_t) *avg_cat += output[index + n];
        }
    }
}
    ...
    for (b = 0; b < l.batch; ++b) {
        if(l.softmax_tree){
            ...
            for(t = 0; t < 30; ++t){
                ...
                if(!truth.x) break;
                int class_t = state.truth[t*5 + b*l.truths + 4];
                float maxp = 0;
                int maxi = 0;
                if(truth.x > 100000 && truth.y > 100000){
                    for(n = 0; n < l.n*l.w*l.h; ++n){
                        ...
                        float p = scale*get_hierarchy_probability(l.output + index, l.softmax_tree, class_t);
                        if(p > maxp){
                            maxp = p;
                            maxi = n;
                        }
                    ...
                    delta_region_class(l.output, l.delta, index, class_t, l.classes, l.softmax_tree, l.class_scale, &avg_cat);
        ...
        for(t = 0; t < 30; ++t){
            ...
            int class_t = state.truth[t*5 + b*l.truths + 4];
            if (l.map) class_t = l.map[class_t];
            delta_region_class(l.output, l.delta, best_index + 5, class_t, l.classes, l.softmax_tree, l.class_scale, &avg_cat);
            ++count;
            ++class_count;
        }
    }
