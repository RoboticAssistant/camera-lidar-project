...
list *get_paths(char *filename)
{
    ...
    if(!file) { printf("get_paths(): Filename: %s\n", filename); file_error(filename); }
...
box_label *read_boxes(char *filename, int *n)
{
    ...
    FILE *file = fopen(filename, "r");
    if(!file) { printf("read_boxes(): Filename: %s\n", filename); file_error(filename); }
    float x, y, h, w;
    int id;
    int count = 0;
...
data load_cifar10_data(char *filename)
{
    ...
    if(!fp) { printf("load_cifar10_data(): Filename: %s\n", filename); file_error(filename);}
    for(i = 0; i < 10000; ++i){
...
data load_all_cifar10()
{
    ...
    for(b = 0; b < 5; ++b){
        char buff[256];
        sprintf(buff, "src/object_depth_module/data/cifar/cifar-10-batches-bin/data_batch_%d.bin", b+1);
        FILE *fp = fopen(buff, "rb");
        if(!fp) { printf("load_all_cifar10(): Filename: %s\n", buff); file_error(buff); }
        for(i = 0; i < 10000; ++i){
            unsigned char bytes[3073];
            fread(bytes, 1, 3073, fp);
            int class_t = bytes[0];
            y.vals[i+b*10000][class_t] = 1;
            for(j = 0; j < X.cols; ++j){
...
data load_go(char *filename)
{
    ...
    if(!fp) { printf("load_go(): Filename: %s\n", filename); file_error(filename); }
    char *label;
    int count = 0;
