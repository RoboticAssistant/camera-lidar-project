...
void train_compare(char *cfgfile, char *weightfile)
{
    ...
    int imgs = 1024;
    list *plist = get_paths("src/object_depth_module/data/compare.train.list");
...
void validate_compare(char *filename, char *weightfile)
{
    ...
    list *plist = get_paths("src/object_depth_module/data/compare.val.list");
    //list *plist = get_paths("src/object_depth_module/data/compare.val.old");
    char **paths = (char **)list_to_array(plist);
    int N = plist->size/2;
    free_list(plist);
...
int bbox_comparator(const void *a, const void *b)
{
    ...
    free_image(im2);
    free(X);
    if (predictions[class_t*2] > predictions[class_t*2+1]){
        return 1;
    }
    return -1;
}

void bbox_update(sortable_bbox *a, sortable_bbox *b, int class_t, int result)
{
    int k = 32;
    float EA = 1./(1+pow(10, (b->elos[class_t] - a->elos[class_t])/400.));
    float EB = 1./(1+pow(10, (a->elos[class_t] - b->elos[class_t])/400.));
    float SA = result ? 1 : 0;
    float SB = result ? 0 : 1;
    a->elos[class_t] += k*(SA - EA);
    b->elos[class_t] += k*(SB - EB);
}

void bbox_fight(network net, sortable_bbox *a, sortable_bbox *b, int classes, int class_t)
{
...
void SortMaster3000(char *filename, char *weightfile)
{
    ...
    list *plist = get_paths("src/object_depth_module/data/compare.sort.list");
    //list *plist = get_paths("src/object_depth_module/data/compare.val.old");
    char **paths = (char **)list_to_array(plist);
    int N = plist->size;
    free_list(plist);
...
void BattleRoyaleWithCheese(char *filename, char *weightfile)
{
    ...
    for (class_t = 0; class_t < classes; ++class_t){

        N = total;
        current_class = class_t;
        ...
        for(round = 1; round <= 100; ++round){
            ...
            for(i = 0; i < N/2; ++i){
                bbox_fight(net, boxes+i*2, boxes+i*2+1, classes, class_t);
            }
            qsort(boxes, N, sizeof(sortable_bbox), elo_comparator);
        ...
        char buff[256];
        sprintf(buff, "results/battle_%d.log", class_t);
        FILE *outfp = fopen(buff, "w");
        for(i = 0; i < N; ++i){
            fprintf(outfp, "%s %f\n", boxes[i].filename, boxes[i].elos[class_t]);
        }
        fclose(outfp);
    }
